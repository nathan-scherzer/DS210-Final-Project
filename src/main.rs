...
fn main() -> Result<(), Box<dyn Error>> {
    let data = read_csv("/opt/app-root/src/Project/all_game_stats.csv")?;

    // Do something with the data
    for row in data.iter().take(5) { // Print first 5 rows
        println!("{:?}", row);
    }
    ...
    //let std_devs = compute_std_devs(&data);
/*
    let offense_benchmarks: Vec<(f64, f64, bool)> = vec![
        (30.0, 6.880765, true), //shots
